...
/** Función para eliminar permanentemente un equipo (Solo Admin) */
function solicitarEliminarEquipo() {
  if (!equipoActualId) {
    alert("Error: No hay equipo seleccionado para eliminar");
    return;
  }
  ...
  mostrarDialogConfirmar(
    ...
    `¿Estás seguro de que deseas eliminar permanentemente el equipo "${equipo.nombre}"? Esta acción borrará su información principal y no se puede deshacer.`,
    async () => {
      try {
        const formData = new FormData();
        formData.append("id", equipoActualId);

        const response = await fetch("../php/eliminar_equipo.php", {
          method: "POST",
          body: formData,
        });
        ...
        if (result.success) {
          ...
          regresarInventario();
          mostrarAlertaIP("success", "✅ Equipo Eliminado", result.mensaje);
        } else {
          mostrarAlertaIP("error", "Transacción Rechazada", result.error || "Permisos insuficientes o error de seguridad.");
        }
      } catch (error) {
